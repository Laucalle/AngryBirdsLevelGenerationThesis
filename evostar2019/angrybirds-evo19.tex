...
structures very close to reality in its dynamic behavior. The main challenge from 
...
From them, we could extract two main approaches to the problem: constructive and search-based.
...
\cite{stephenson2017generating,stephenson2016procedural} the
structures displayed on the level are constructed from the top down,
in several phases. 
...
is the chosen for the level. The probability table was tuned using search-based optimization
methods. % All this could probably be made shorter

The main problem with this construtive approach is that the range of
...
levels. % Same game clone as we use here? - JJ
...
includes the distances between different columns. The population is initialized randomly following a probability table which 
defines the likelihood of a certain element being placed in a certain position 
inside a column. This implies that a certain shape is chosen
beforehand, once again to guarantee stability, but decreases
...
Since we are focusing in this paper on free form evolution, we will
mainly have to look outside the game field to focus on optimization in
architecture, where there are several  approaches to structural 
optimization using search based algorithms. Gandomi et al. \cite{gandomi2013cuckoo} proposed a bioinspired algorithm called Cuckoo Search and 
tested its performance with several structural optimization problems. The 
...
\cite{hornby2001advantages}. In this case, structures are formed by voxels and 
build following instructions given as commands to a LOGO-style turtle. The 
structures evolved in this experiments were tables. The commands are generated 
by a context-free L-system. For the evolutionary algorithm, an L-system and its 
production rules are considered individuals. In order to allow these systems to 
evolve they are parametrized and constrained.
...
generated, but stil precludes the formation of disjoint structures
(for instance, a defensive tower before a simple pigsty). 
...
The main open source Angry Birds simulator is the one that had been developed by  Ferreira and 
Toledo \cite{ferreira2014search}, available in
GitHub\cite{sciencebirds}. We used it  as a starting point; however we
had to patch it for producing usable output and automate its work. The modified 
...
ready, we will have to design that fitness function. As obvious as it
...
The (modified) game simulator provides the average magnitude of
...
For that 
matter, before testing a level, there are some indicators, like
distance to the ground and number of overlapping blocks, that a level would 
not be suitable and thus simulation can be skipped.
...
Engine underlying the simulator will 
solve the issue moving the blocks until there is no collision, but
this is done by Unity proprietary code and it is impossible to know
what it does.
% Since Unity 
...
% inaccurate.
So, a penalization is applied and the level is not simulated either. 
...
objective, we performed a series of experimentes presented in next
section. 
...
\begin{table}[H]
	...
	\begin{tabular}{ccccc}
		...
		\hline
	\end{tabular}
	\caption{Summary of results, G: number of generations} % What
...
population) or best fitness value below 0.01. % This should not be
                                % "also", it's maybe the most
                                % important change - JJ
...
used which was described in section \ref{ga:cross2}. % This reference
...
competitive engry for the Angry Birds level generation contest, and  a
set of subobjectives in mind: The first 
goal was to explore the expressiveness and variability of SBPCG with
evolutionary techniques. Perhaps the level of achievement of this one
...
movement or maximum estability results in structures that are close to
