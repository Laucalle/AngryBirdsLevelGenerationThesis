...
\textit{3rd Angry Birds Level Generation Competition}. %reference needed.
Participants must build 
computer programs that are able to generate levels for the \textit{Angry Birds} 
game.
% Antonio - remove the reference to CIG conference (EvoGAMES is a similar 
%conference). Just introduce the Level Generation Competition.
% Antonio - You should first present the game and later introduce the 
%competition centered on that game
...
% This is said in Togelius paper that is already cited and I haven't found much 
% more on the topic - Laura
The problems faced by SBPCG are not very far from those encountered in
Evolutionary problems; since evolutionary algorithms are search
methods, they can be a good match to perform this kind of procedures. 

It is probably well known that, in the game of Angry Birds, birds are
launched against structures that include pigs. What we are searching
for are these structures, whose playability and appearance has to be
optimized. But first of all, they are structures with a realistic
gravity, which must not break down when they are erected at the
beginning of a level. We will have to include this factor into the 
\textit{fitness function}, which grades how good a solution is, and
frequently, it punishes non-valid solutions.  
...
Outside game development, we can find some approaches to structural 
optimization using search based algorithms. Amir Hossein Gandomi, Xin-She Yang 
and Amir Hossein Alavi proposed a genetic algorithm called Cuckoo Search and 
...
structural optimization is not flexible enough to fit our optimization problem. 

A different take on the design of structures is using Generative Grammatical 
Encodings as Gregory S. Hornby and Jordan B. Pollack present in 
...
parts of the other one in it, like a subsequence of commands for example.
