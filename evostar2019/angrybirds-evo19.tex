...
Birds, with the ultimate objective of creating levels with the minimum
number of constraints. This approach consists of a search-based procedural level 
...
%was celebrated in 2018 during the conference on Computational Intelligence and Games (CIG)\cite{aibirds}.
%Participants must build 
%computer programs that are able to generate levels for the \textit{Angry Birds} game;
%Generated levels must first be valid (i. e. not crash), and then,
%they are evaluated and judged for playability and aesthetics. 

% This is mentioned in the next section - Laura
...
% base for later development, enhancing human creativity rather than displacing it.
...
\textit{generate-and-test} %---it produces potentially correct solutions that are tested and adjusted in each iteration before giving the actual output---
...
usually tackled with Evolutionary Algorithms(EA) \cite{togelius2010search} like the 
...
%for these structures, whose playability and appearance has to be optimized.
...
%since it actually involves the graphic
...
\begin{itemize}
	...
	with distinct shapes or sizes.% they are represented as an integer between $0$ and $7$.
	...
	rotations are considered, 0, 45, 90 or 135 degrees.% represented as integers between $0$ and $3$.
...
stored, so
%that the value of not tested levels is always above ---it is a minimization problem--- the in-game 
%tested levels; 
...
\begin{itemize}
	...
	%position must be multiple of the dimensions of the smallest block (
...
\begin{itemize}
	...
	so it adds or subtracts one to the current value. % If types follow a logical order, you
...
frameworks
%such as DEAP \cite{fortin2012deap}, PyEvolve \cite{perone2009pyevolve} or Evolopy \cite{evolopy}
...
\begin{table}[H]
	...
	\begin{tabular}{cclclclclcl}
		...
		% added standard deviation - Laura
		\hline
	\end{tabular}
...
\begin{figure}[H]
	\centering
	\subfloat[One of the results for \ref{E1}: Fitness = 9.021, nBlocks = 7, G = 100  \label{f:e1}]{%
		\includegraphics[scale=0.2]{level-0-180523_203106.png}
	}
	\hfill
	\subfloat[One of the results for \ref{E2}: Fitness = 5.176, nBlocks = 9, G = 767  \label{f:e2-4}]{%
...
\begin{figure}[H]
	\centering
	\subfloat[One of the solutions from \ref{E3}: Fitness = 1.571e-7, nBlocks = 9, G = 194  \label{f:e3-4}]{%
		\includegraphics[scale=0.2]{level-0-second_crossover180613_055622.png}
	}
	\hfill
	\subfloat[One of the solutions from \ref{E4}: Fitness = 9.229e-7, nBlocks = 10, G = 198 \label{f:e4-1}]{%
...
shown in Figure \ref{f:e5}.
 ...
 \begin{figure}
 	\centering
 	\subfloat[One of the solutions from \ref{E5}\label{f:e5}]{%
 			\includegraphics[scale=0.27]{E5.png}
 	}
 	\hfill
 	\subfloat[One of the solutions from \ref{E6}\label{f:e6}]{%
 			\includegraphics[scale=0.27]{E6.png}
 	}
 \end{figure}
