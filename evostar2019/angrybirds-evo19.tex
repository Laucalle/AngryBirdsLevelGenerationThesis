...
\titlerunning{Evolution of Angry Birds Levels}
...
soundness. Different representations and operators have been considered and studied. In order to test the method four experiments have been carried out,
...
The ultimate objective of the project presented in this paper is to
...
usually tackled with Evolutionary Algorithms (EAs) \cite{togelius2010search} like the one proposed in \cite{hastings2009evolving}. 
The problems faced by SBPCG are not very far from those encountered in
EAs; since these algorithms can be used as search methods, they can be a good match to perform this kind of procedures. 

Given that in Angry Birds birds are launched against structures that include pigs, we will be searching for these structures, whose playability and appearance has to be optimized. We try to address playability by eliminating the constraint that they must follow a pattern, and do not really address aesthetic aspects in this paper, not even constraining the structures to be
...
PCG is becoming more frequently used in new games as one of the main tools for supporting designers and developers. It has a growing relevance which has been also translated into the arising of new international competitions aimed to design algorithms to generate interesting/funny/enjoyable contents for different games, such as Super Mario (a clone of it indeed) \cite{MarioAI_Level_12}, not specified General Video Games \cite{GAIG_LevelGeneration_18,Khalifa_GVGLG_16}, or recently, for Angry Birds \cite{AngryBirds_LevelGeneration_18}.

Obviously, all the entries presented in previous editions of the Angry Birds Level Generation Competition are strongly related with this work. Unfortunately just a few of authors have published their proposals in conferences or journals.

2018 edition of the competition \cite{AngryBirds_LevelGeneration_18} was focused on finding the best entry generating funny levels. Thus, fun was the main factor in the evaluation of the participants, then creativity and difficulty were also taken into account as secondary factors.
Six entries competed on it, being the most representative: the one by J. Yuxuan et al., able to generate random quotations and formulae with the different components of a level; J. Xu et al. proposal which generates levels that look like pixel images; or a third approach (by C. Kocaogullar) that generates levels by translating music patterns to structures.

The winner was a proposal called Iratus Aves, an iteration of the works by M. Stephenson and J. Renz \cite{stephenson2017generating,stephenson2016procedural}, which is a \textit{constructive method} where the structures displayed on the level are built from top to down, in several phases. 
The followed process builds a structure recursively, each row composed of a single type of block (with a fixed rotation). The likelihood of selecting a certain block is given by a probability table, which was tuned using an optimization method. Then, the blocks are placed using a tree structure, where the first selected block is the peak and blocks underneath it are split into subsets that support the previous row with one, two or three blocks. This ensures local stability, but not global stability, which is tested once the whole structure is completed. 
After that, other objects (pigs and TNT) are placed, following an scheme of potential positions for them according to a rank based on some terms such as structural protection or dispersion.

This approach was improved adding a `layer' devoted to the selection of the material (stone, ice or wood) based on different strategies. Materials are important given that certain birds are more efficient against particular ones, so they will have a high impact on the game development. Thus, a strategy sets stone to detected weak points. Trajectory analysis-based strategy sets to the same material all blocks in the trajectory of a shot aimed at a particular pig. Clustering strategy takes a random block, sets its material and propagates to the surrounding blocks that have not been assigned yet. Row grouping and structure blocking apply the same material to a whole row or structure respectively.

% All this could probably be made shorter
% Antonio - I have reduced and better connected it. ;)

The main problem with this constructive approach is that the range of
...
\textit{Search-based approaches} propose a good alternative to deal with these limitations. Thus, Lucas Ferreira and Claudio Toledo \cite{ferreira2014search} presented a solution -- ranked 4th on the last competition -- based on SBPCG, which uses a Genetic Algorithm (GA) and a game clone named \textit{Science Birds} developed to evaluate the levels. 
% Same game clone as we use here? - JJ
% yes - Laura

In the GA individuals correspond to levels, each represented by an array of columns. Each column is a sequence of blocks, pigs and predefined 
compound blocks, using an identification integer. This representation also 
includes the distances between different columns. The population is initialized randomly following a probability table which defines the likelihood of a certain element being placed in a certain position inside a column. This implies that a shape is chosen beforehand, once again to guarantee stability, but decreases
playability by generating structures whose only differences are which
blocks are placed on top of which. 
Levels are evaluated executing them in the simulator and checking their average stability, considering the speed of every block when erected -- which must tend to be zero for having a stable structure --. The authors designed specialized crossover and mutation operators, aiming to maintain some consistency in the new solutions generated.
%
%
...
%of elements. % This is the velocity after they have been hit by a
             % bird? Or when they are erected? - JJ
             % Velocity when erected. They don't use agents at this point - 
             % Laura
...
% Antonio - We normally don't describe so many details in the SotA, unless their are very relevant for the paper purposes, but I think this is not the case, so I have summarized this in a sentence (also saving some space). 
...
% In general. The state of the art has to reflect what other people are doing 
% in general - JJ
% This paragraph above can probably be suppressed. Since they are not
% using a free form, this is not relevant for this paper - JJ

% Antonio - Briefly comment if this paper follows a similar approach to Lucas's GA

However, this paper proposes a different approach: the \textit{free form evolution}. To our knowledge there are not many examples of similar proposals in the games field. 
% Antonio - This is the main selling point of the paper, so if you think this is the only approach of free form evolution in games, remove the word 'many' in the sentence above. ;D
Thus, we will mainly have to look outside the game field to focus on optimization in architecture, where there are several approaches to structural optimization using search-based algorithms. Gandomi et al. \cite{gandomi2013cuckoo} proposed a bioinspired algorithm called Cuckoo Search and tested its performance with several structural optimization problems. The 
...
%
% Certain cuckoo species lay eggs in other birds' nests and if the new host does 
% discover the intruding eggs it can either abandon the nest or throw them away. 
% Keeping this behaviour in mind, the Cuckoo Search follows a set of rules to 
% balance between exploration and exploitation. A cuckoo represents a 
% solution. Each of them can lay one egg and place it in a random nest. The nests 
% that contain the best eggs continue to the next generations. A host can 
% discover an egg can with a certain probability throw it away or start a new 
% nest elsewhere. The total number of nests is fixed.
...
Encodings as Hornby and Pollack present in \cite{hornby2001advantages}. In this case, structures are formed by voxels and build following instructions given as commands to a LOGO-style turtle. They implementad an EA which evolved tables as structures. The commands are generated by a context-free L-system. For the EA, an L-system and its production rules are considered individuals. In order to allow these systems to evolve they are parametrized and constrained.
%
% Production rules are formed by different components: the predecessor, the 
% condition and the successor, like in the following example:
% $$A(n0, n1): n1 > 5 \rightarrow B(n1, n0)$$
% This algorithm restricts the comparison to whether the production parameter is 
% greater than a constant. Parameters to design commands are constant values or 
% production parameters. Parameters to productions are equations that use either 
% production parameters or constants as in :
% $$[production \; parameter | constant ]\; [ + | - | \times | 
% \; \backslash]\; [ production \; parameter | constant ]$$
% --- No more details are needed on this kind of section --
%
...
% Antonio - It is required a sentence relating these works with the one developed here. It could tell about similarities or differences. The aim is introduce/justify the contribution of the paper to the State of the Art. Which could be the novelty of using of free form to a a new scope.
...
and automate its work. The modified version is available on GitHub \cite{sciencebirds-adapt}.
...
The modified version of the game simulator provides the average magnitude of
...
decided to take into account additional factors on the fitness so that not all
...
not be a valid solution and thus simulation can be skipped.
...
% This is obvious. You should instead create a narrative of the
% experiments performed, and why every experiment leads to the next -
% JJ

% This table should probably be moved to the end of the section - JJ
Four experiments have been carried out, with more than 15 executions per 
experiment. 
% Antonio - say exactly how many runs have you conducted
Table \ref{t:resOver} shows an overview of the results. 
...
% When you show average results, you must also add standard deviations in the shape $\pm$ 123.45.
\begin{table}[H]
	\myfloatalign
	\begin{tabular}{ccccc}
...
% If possible, add standard deviation here - JJ
		\hline
	\end{tabular}
	\caption{Summary of results, G: number of generations, E: experiment 
	number} % What
...
\end{figure}
% Antonio - Comment why are you showing the worst individual
...
% Antonio - Again, you must explain the aim of the experiment at the beginning (you must do it in every one)
...
The rest of the operators as well as the termination criteria are kept the same. 
...
% Antonio - Add a sentence describing the aim of the experiment
...
best fitness value increased slightly, while the average and worst values are lower. This suggest that the latest generations of this EA are less diverse than those from the third experiment.
...
% Antonio - I think it would be a very good idea to include an example of structure generated by your method. It would be even better if you could show the evolution of the structures in some generations (maybe the best in every generation). Let's say you show 4: one from the initial population, at generation 300, at generation 600, and at the end of the evelutionary process.
% This is always attractive for the reviewers and a graphical demonstration of your method working. ;D
...
This project was developed with the main objective of developing a
...
This work was developed with the initial objective of developing a
competitive entry for the Angry Birds level generation contest, and a
secondary objective in mind: exploring the expressiveness and variability of SBPCG with evolutionary techniques. 
%Perhaps the level of achievement of this one
%is not as easily measurable as the other three. It could seem like
...
%the project to test SBPCG in general, but in this particular case, for
%this particular game. Considering this, 
% Antonio - Do not say what you haven't done
For this aim we have implemented and Evolutionary Algorithm able to optimize level structures to meet some criteria, being the main one the stability of the constructions. The method studied let us conclude that SBPCG methods are a potential good solution to offline content generation but it requires a great amount of problem-specific knowledge. Like any other form of creative work, the biggest issue may be how to measure how good, creative or enjoyable is the piece. The more rules the author adds, expressiveness starts to get lost as the
results are variations of the same idea. However, it is crystal clear
from the experiments run in this project that a lack of knowledge
will lead to unexpected and even disappointing outcomes. % I am lost
                                % in this paragraph. It should be more
                                % related to the actual experiments
                                % made - JJ
% Antonio - I have reduced and focused a bit more the text
...
%right now, the game would have to be changed and compiled again. It
%would be interesting to obtain other kind of data from the simulator,
...
% Antonio - Again, in a paper conclusions you must remark what you have done. ;)


Producing stable structures under gravity was the third objective and it has been effectively achieved, but the consequence of evolving in a path of minimum
movement or maximum stability results in structures that are close to
the ground and do not have more than a few floors, which could not be very exciting for the players. 
% Antonio - I have expressed my opinion about this.
% Tell me what you think of this.
The results are strongly related with the main issue of this approach, which was the proper evaluation of the levels. Since we define as level a structure that is stable, evolution will maximize stability finding, as in the beginnings of architectural practice, squat structures that are neither aesthetically pleasing nor playable, although undeniably sturdy and stable.


%In fact this is a matter of how we define what an Angry Birds' level \textit{is} and if that definition matches the fitness function. A lot of elements were correct, but the definition was not complete. How this definition of level plays
...
% Antonio - I have rewriten in part this above

Thus, we have been successful in evolving free form structures to find these type. But once we get there, we need to go into a different evolution mode that takes into account several features as a multiobjective (or aggregated) optimization problem: stability first, but also height or some other aesthetic factors.
...
%So, as a conclusion this work provides an interesting insight 
%into the SBPCG, through the completion---and failure---of the goals we
%set out to achieve at the beginning.

% Antonio - it is not possible to sell the publication of an unsuccessful paper, so you must mainly focus and defend the good parts. Bas parts must be presented as they will be addressed and improved in future work. ;D
...
Another important issue that will be addressed is the time performance. 
...
% Antonio - Do not mention future problems. ;D
