...
blocks which house {\em pigs} %(which we can call {\em pigsties} for that reason)
...
in C++ originally used for the actual Angry Birds game, to avoid launching
the whole game. In this simulation we do not have the resistance of the
...
We will try these solutions to allow a less directed search than previous 
solutions while keeping a 
simple representation. 

Individuals are composed by a list of blocks; platforms, TNT boxes or pigs are 
...
Candidates for reproduction are selected using tournaments. Two individuals are 
chosen from the 
